...
async fn monitor_user_cancellation_does_not_wake_agent() -> anyhow::Result<()> {
    ...
    let mock = mount_sse_sequence(
        ...
        vec![
            call(
                "start",
                json!({"command": watcher(), "description": "cancel me"}),
            ),
            done("idle"),
        ],
    )
    ...
    assert_eq!(
        terminals[0].monitor_description.as_deref(),
        Some("cancel me")
    );
    assert!(
        harness
            .test()
            .codex
            .terminate_background_terminal(terminals[0].process_id.parse()?)
            .await
    );
    harness.write_file("signal", "ready").await?;
...
async fn monitor_flood_stops_and_releases_capacity() -> anyhow::Result<()> {
    ...
    let flood = match test_target_os() {
        TestTargetOs::Linux | TestTargetOs::MacOs => {
            "while ! test -f signal; do sleep 0.05; done; yes EVENT"
        }
        TestTargetOs::Windows => {
            "while (!(Test-Path signal)) { Start-Sleep -Milliseconds 50 }; while ($true) { Write-Output ('EVENT' * 1000) }"
        }
    };
    let started = mount_sse_sequence(
        harness.server(),
    ...
    assert!(
        request
            .message_input_texts("user")
            .iter()
            .any(|t| t.contains("Stopped:") && t.len() < 900)
    );
    assert!(
        harness
            .test()
...
async fn subagent_cannot_create_monitor() -> anyhow::Result<()> {
    ...
    let mock = mount_sse_sequence(
        ...
        vec![
            call(
                "start",
                json!({"command": watcher(), "description": "forbidden"}),
            ),
            done("done"),
        ],
    )
    ...
    spawned
        ...
        .await?;
    wait_for_event(&spawned.thread, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
    let requests = mock.requests();
