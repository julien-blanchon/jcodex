...
impl App {
    pub(super) async fn manage_monitors(
        ...
        thread_id: ThreadId,
        stop_process_id: Option<&String>,
    ) -> Result<()> {
        let handle = app_server.request_handle();
        if let Some(process_id) = stop_process_id {
            let response: ThreadBackgroundTerminalsTerminateResponse = handle.request_typed(
            ...
            self.chat_widget.add_info_message(
                if response.terminated { "Monitor stopped." } else { "Monitor has exited or could not be stopped." }.into(),
                /*hint*/ None,
            );
        }
        let mut cursor = None;
        ...
        loop {
            let response: ThreadBackgroundTerminalsListResponse = handle.request_typed(
        ...
        if items.is_empty() {
            self.chat_widget.add_info_message("No active monitors in this thread.".into(), /*hint*/ None);
        } else {
...
fn monitor_items(thread_id: ThreadId, terminals: Vec<codex_app_server_protocol::ThreadBackgroundTerminal>) -> Vec<SelectionItem> {
    let mut items = Vec::new();
            for terminal in terminals {
                let Some(description) = terminal.monitor_description else { continue };
                ...
                items.push(SelectionItem {
                    ...
                    actions: vec![Box::new(move |tx| {
                        tx.send(AppEvent::SubmitThreadOp {
                            thread_id,
                            op: AppCommand::StopMonitor { process_id: process_id.clone() },
                        });
                    })],
